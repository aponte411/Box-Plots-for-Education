...
train_df = pd.read_csv('Boxplots_train.csv', encoding='Latin-1', index_col=0)
test_df = pd.read_csv('Boxplots_test.csv', encoding='latin-1', index_col=0)
...
for i in range(1, len(sample.columns)):
    ...
    submission[col] = preds[:,i-1] # add predictions to submission
    
# save as csv
submission.to_csv('SGD_Model.csv', index = False)
